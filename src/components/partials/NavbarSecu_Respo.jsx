...
import { FaBell, FaBullhorn, FaPhoneAlt, FaUsers } from 'react-icons/fa';
import Avatar from '../common/Avatar';
import logo from '../../assets/Logo.png';

export const NavbarSecu_Respo = () => {
    ...
    <nav className="fixed top-0 w-full lg:px-16 px-6 flex flex-wrap items-center lg:py-0 py-2 bg-amber-950 shadow-md z-50">
      <div className='w-full container mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8 md:h-14 h-12'>
        ...
        <div className='flex-1 flex justify-between items-center'>
          {/* Logo Section */}
          <Link to="/Secu_RespoLayout" className='flex items-center gap-1 sm:gap-2'>
      ...
      >
        <ul className="flex flex-col pt-3 pb-3 gap-3 text-lg font-semibold px-4">
          <li><Link to="/Secu_RespoLayout/profile" onClick={() => setIsMobileMenuOpen(false)} className='flex text-base items-center gap-3 text-amber-950 hover:text-amber-700 transition-colors rounded-md px-2 py-1 focus:outline-none focus:ring-2 focus:ring-amber-500' role="menuitem"><MdAccountCircle size={18} />Profile</Link></li>
          <li><Link to="/Secu_RespoLayout" onClick={() => setIsMobileMenuOpen(false)} className="flex text-base items-center gap-3 text-amber-950 hover:text-amber-700 transition-colors rounded-md px-2 py-1 focus:outline-none focus:ring-2 focus:ring-amber-500" role="menuitem"><FaBullhorn size={18} />Announcements</Link></li>
          <li><Link to="/Secu_RespoLayout/emergencyLogs" onClick={() => setIsMobileMenuOpen(false)} className="flex text-base items-center gap-3 text-amber-950 hover:text-amber-700 transition-colors rounded-md px-2 py-1 focus:outline-none focus:ring-2 focus:ring-amber-500" role="menuitem"><FaPhoneAlt size={18} />Emergency Logs</Link></li>
          <li><Link to="/Secu_RespoLayout/visitorLogs" onClick={() => setIsMobileMenuOpen(false)} className='flex text-base items-center gap-3 text-amber-950 hover:text-amber-700 transition-colors rounded-md px-2 py-1 focus:outline-none focus:ring-2 focus:ring-amber-500' role="menuitem"><MdListAlt size={18} />Visitor Logs</Link></li>
          <li><Link to="/Secu_RespoLayout/officials" onClick={() => setIsMobileMenuOpen(false)} className='flex text-base items-center gap-3 text-amber-950 hover:text-amber-700 transition-colors rounded-md px-2 py-1 focus:outline-none focus:ring-2 focus:ring-amber-500' role="menuitem"><FaUsers size={18} />Officials Board</Link></li>
          <li><button onClick={confirmLogout} className='flex text-base items-center gap-3 text-amber-950 hover:text-amber-700 transition-colors rounded-md px-2 py-1 focus:outline-none focus:ring-2 focus:ring-amber-500' role="menuitem"><MdLogout size={18} />Log out</button></li>
          </ul>
      </div>
